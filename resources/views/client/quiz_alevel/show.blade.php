 ...
 <div class="content-wrapper">
    ...
    <div class="content">
        <div class="container">
          ...
          @else
          @foreach ($quizAlevels as $quizAlevel)
          <div class="card card-warning card-outline">
            <div class="card-body">
              <h4 class="card-title"><strong>Name:</strong> {{ $quizAlevel->name }}</h4><br>
              <p><strong>Time:</strong> {{ $quizAlevel->time }} Min </p>
              <hr>
        
              <p class="card-text">{{ $quizAlevel->description }} </p>
              {{-- @if ($quizAlevel->total_marks == "") --}}
              <a href="/client/alevel/quiz/{{ $quizAlevel->subject }}/take/{{ $quizAlevel->id }}" class="card-link btn btn-sm btn-danger">Take Quiz</a>
              {{-- @else --}}
              <a href="#" rid="{{ $quizAlevel->id }}" class="card-link btn btn-sm btn-secondary view_results">View results</a>  
              {{-- @endif --}}
          ...
          @endif 
           ...
           {{ $quizAlevels->onEachSide(1)->links() }} 
