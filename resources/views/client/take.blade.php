          ...
          $.ajax({
            ...
            success:function(res)
            {
              ...
              if (res.question.quizOlevel.questions_olevel[0] == null) {
                ...
                $('#question_name').html('<strong>Qn: </strong>'+res.question.quizOlevel.questions_olevel[0].question_name);
                $.each(res.question.quizOlevel.questions_olevel[0].choice_olevel, function(key, value){
                    $('#choice_area').append('\
                    <div class="form-check mb-1">\
                      <input class="form-check-input" type="radio" name="choice"  value='+res.question.quizOlevel.questions_olevel[0].choice_olevel[key].id+'>\
                      <label class="form-check-label" for="exampleRadios1">'+value.text+'</label>\
                    </div>'); 
                  });
                  $('#button_area').html('\
                  <button id="prev" class="btn btn-sm btn-warning previous"  pid='+res.question.quizOlevel.questions_olevel[0].id+'>Prev</button>&nbsp;\
                  <button id="next_btn" class="btn btn-sm btn-info next" nid='+res.question.quizOlevel.questions_olevel[0].id+'>Next</button>');
